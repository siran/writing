    ...
    for it in items_sorted:
        ...
        if it.is_dir:
            ...
            if ext in MD_EXTS:
                ...
                # PDF link: local PDF (generated by podman) takes priority, then provenance URL.
                local_pdf = mirrored.with_suffix(".pdf")
                if local_pdf.exists():
                    local_pdf_url = "/" + quote(rel_out(local_pdf).as_posix(), safe="/:@-._~")
                    extra.append(f'<a href="{html.escape(local_pdf_url, quote=True)}">pdf</a>')
                else:
...
# Root-level files generated fresh each build (no direct ROOT source).
...
def _out_has_source(out_path: Path) -> bool:
    ...
    Generated index.html / book-html artifacts produced from a book.yml are
    recognized via the presence of book.yml/book.yaml in the containing dir.
    """
    ...
    if name.endswith(".pandoc.md"):
        stem = name[: -len(".pandoc.md")]
        md_src = ROOT / rel_p.with_name(stem + ".md")
        if md_src.exists():
            return True
...
def prune_orphans_from_out():
    """Sweep OUT and remove files whose sources no longer exist.

    This catches content that was generated by previous builds but whose
    source has been deleted from ROOT since. Without this pass, deleted
    sources leave stale files in OUT until a manual clean.
    """
    removed = 0
    for path in sorted(OUT.rglob("*"), key=lambda p: len(p.parts), reverse=True):
        if not path.exists() or not path.is_file():
            continue
        rel_p = path.relative_to(OUT)
        parts = rel_p.parts
        name = path.name

        # Root-level generated files (sitemap, robots, feed, top index.html, etc.)
        if len(parts) == 1 and name in _GENERATED_ROOT_FILES:
            continue

        # .well-known/ stays (RFC 8615; verification files etc.)
        if parts and parts[0] == ".well-known":
            continue

        # /doi/ aliases (root or nested under a top-level like prints/doi/)
        # are redirect pages generated by build_article_pages(). They have
        # no source counterpart and typically contain only an index.html
        # with no siblings -- so the fallback source check would delete
        # them. Keep anything whose path includes a 'doi' segment.
        if "doi" in parts:
            continue
        ...
        try:
            path.unlink()
            removed += 1
            print(f"[PRUNE] removed orphan: {rel_p}")
        except Exception as e:
            print(f"[PRUNE] failed to remove {rel_p}: {e}")

    # Sweep empty directories bottom-up.
    for path in sorted(OUT.rglob("*"), key=lambda p: len(p.parts), reverse=True):
        if not path.exists() or not path.is_dir() or path == OUT:
            continue
        try:
            next(path.iterdir())
        except StopIteration:
            try:
                path.rmdir()
                print(f"[PRUNE] removed empty dir: {path.relative_to(OUT)}")
            except Exception:
                pass

    if removed:
        print(f"[PRUNE] total orphan files removed: {removed}")
