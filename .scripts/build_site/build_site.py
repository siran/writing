    ...
    for it in items_sorted:
        ...
        if it.is_dir:
            ...
            if ext in MD_EXTS:
                ...
                # PDF link priority:
                # 1. Local PDF in OUT (generated by podman) → serve from GitHub Pages
                # 2. .pdf marker covers this file → GitHub blob URL (renders inline)
                # 3. provenance URL
                ...
                if local_pdf.exists():
                    ...
                    pdf_link_url = None
                    # Check if source .pdf marker covers this file
                    src_file = ROOT / rel_out(it.path)
                    pdf_src_marker = src_file.parent / ".pdf"
                    if pdf_src_marker.exists():
                        try:
                            _mt = pdf_src_marker.read_text(encoding="utf-8").strip()
                        except Exception:
                            _mt = ""
                        _covered = not _mt or src_file.name in {
                            ln.strip() for ln in _mt.splitlines() if ln.strip()
                        }
                        if _covered and OWNER and REPO and BRANCH:
                            _pdf_gh_path = quote(
                                src_file.with_suffix(".pdf").relative_to(ROOT).as_posix(),
                                safe="/:@-._~",
                            )
                            pdf_link_url = (
                                f"https://github.com/{OWNER}/{REPO}/blob/"
                                f"{BRANCH}/{_pdf_gh_path}"
                            )
                    if not pdf_link_url:
                        prov_path = it.path.parent / "provenance.yaml"
                        if prov_path.exists():
                            pdf_link_url = _provenance_pdf_url(prov_path)
                    if pdf_link_url:
                        extra.append(f'<a href="{html.escape(pdf_link_url, quote=True)}">pdf</a>')
                if gh_url:
