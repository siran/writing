...
Chapter 10 established that a single toroidal closure carries organized
$(m,n)$ energy that structures its enclosing shells, along with an oriented
axial through-hole flux. For a large aggregate of $N$ such closures with
random orientations, all directed structure cancels: the charge signs are
mixed, the axial fluxes point in all directions. What cannot cancel is the
scalar energy of each closure, which is always positive. The aggregate
therefore carries total energy $Mc^2 = NE$, with no net oriented structure.
This residual energy is spread isotropically over enclosing shells. Its
surface density on a shell at radius $r$ is

$$
\frac{Mc^2}{4\pi r^2} \propto \frac{1}{r^2}.
$$

This exterior loading is unsigned and always additive — scalar energies sum
regardless of orientation. This is the shape of the gravitational field:
isotropic, inverse-square, always attractive.

The weak-field scalar strength of this exterior loading is summarized by the
mass-potential
