...
This appendix develops the weak-field gravity closure used in chapter 13 as far
as the static benchmark set.
...
that one geometry the standard first-order observables:
...
Chapter 13 adopted the symmetric constitutive modification
...
So the constitutive closure determines the local transport speed of weak
electromagnetic disturbances in the background generated by the central mass.

## 212.2 Determination of the Weak-Field Metric

To first order in $\eta$, the most general static spherically symmetric
...
Expand to first order in $\eta$ and $v^2/c^2$:
...
So the constitutive closure of chapter 13 does not merely reproduce light
bending. It fixes the full static weak-field metric at first order.
...
Take a ray passing the mass with impact parameter $b$. To first order, use the
...
To first order in $GM/(Rc^2)$, the metric becomes
...
first order in $\delta$ and $GM/c^2$ yields
...
Starting from the symmetric constitutive closure
...
the unique first-order static isotropic metric
...
So within the adopted symmetric constitutive closure, chapter 13 no longer
rests on light bending alone. The full static weak-field benchmark set is
recovered from one and the same transport geometry.
