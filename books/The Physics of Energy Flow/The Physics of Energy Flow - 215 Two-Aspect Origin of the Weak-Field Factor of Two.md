...
admissible first-order background response must couple to both.
...
Now consider a weak static background generated by a localized massive mode.
Suppose the background response to the probe is:
...
response of a null Maxwell probe.

Within the stated response class, this is the structural origin of the familiar
factor of two:

- a scalar or one-channel model sees only one half,
- a null electromagnetic probe carries two equal stress channels,
- any admissible first-order background response must act on both.
...
solution bend another. The missing object is the background response of the
localized massive closure to the passing null probe.
...
- it shows what any correct first-order background response must do,
- it shows why a one-channel account gives only the Newtonian half-value,
- it shows why the full electromagnetic response must double that result.

## 215.4 Relation to Chapter 13 and Appendix 212

Chapter 13 and appendix 212 summarized the weak background response by the
...
macroscopic summary of a background response that must, if it is correct, act
...
2. a one-channel theory gives only half the response,
3. the full first-order background response must couple to both,
4. the symmetric constitutive summary used in chapter 13 is the macroscopic
   encoding of that doubled response.

## 215.5 What Still Remains Open

The remaining task is not to guess the factor of two. Within the stated
response class, that part is already fixed by the two-aspect structure of the
probe.

The remaining task is to derive the actual background-response operator of a
localized massive closure and to show that it yields the full weak-field
interaction law.

In particular, a complete derivation should:

- start from a bounded mass mode and a passing null probe,
- compute the induced first-order background response,
...
Therefore any admissible first-order background response that treats the two
Maxwell aspects symmetrically must produce

$$
\text{full null response}
=
2\times\text{one-channel response}.
$$
...
What remains open is the exact background-response derivation that turns this
